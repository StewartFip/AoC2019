...
def wirepath(wire):
  ...
  for vector in wire:
    din = vector[0:1].lower()
    dis = int(vector[1:len(vector)])

    for plot in range(1,dis + 1):
      step += 1
...
def manhattandistance(wire1, wire2):
  ...
  matches = set(path1[0]) & set(path2[0])
  dist = None

  for match in matches:
    if dist == None:
      dist = path1[1][path1[0].index(match)] + path2[1][path2[0].index(match)]
    else:
      dist = dist if path1[1][path1[0].index(match)] + path2[1][path2[0].index(match)] > dist else path1[1][path1[0].index(match)] + path2[1][path2[0].index(match)]
