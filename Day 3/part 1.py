...
def manhattandistance(wire1, wire2):
  ...
  matches = set(path1) & set(path2)
  dist = None

  for match in matches:
    if dist == None:
      dist = abs(match[0]) + abs(match[1])
    else:
      dist = dist if abs(match[0]) + abs(match[1]) > dist else abs(match[0]) + abs(match[1])
...
def main():
  ...
  # 22 is incorrect, 2 is incorrect
